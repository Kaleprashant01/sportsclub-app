        ...
        <div id="matches-list">
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                while ($row = mysqli_fetch_assoc($result)) {
                    ...
                    echo "<h2>" . $row['home_team'] . " vs " . $row['away_team'] . "</h2>";
                    echo "<p>Date: " . $row['match_date'] . "</p>";
                    echo "<p>Venue: " . $row['venue'] . "</p>";
                    echo "<p>Created At: " . $row['created_at'] . "</p>";

                    // Display join button if match status is "Open"
                    if ($row['status'] == "Open") {
                        echo "<form method='post' action='join_match.php'>";
                        echo "<input type='hidden' name='match_id' value='" . $row['id'] . "'>";
                        echo "<button type='submit' name='join_match' class='join-button'>Join</button>";
                        echo "</form>";
                    }
